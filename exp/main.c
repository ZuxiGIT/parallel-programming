...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //fprintf(stdout, r"Initial fac = %Lf\n", my_rank, fac);
        ...
        long double fac = start;

        fprintf(stdout, r" Initial fac %Lf\n", my_rank, fac);

        for(int i = start + 1; i <= finish ; i++) 
        {
            //fprintf(stdout, r" i = %d\n", my_rank, i);
            sum += 1. / fac;
            fac *= i;
        }
        ...
        if(my_rank != 1)
            res = MPI_Recv(&prev_res, 1, MPI_LONG_DOUBLE, my_rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        if(res != MPI_SUCCESS)
            fprintf(stderr, r"Error ocurred while receiving from %d to %d\n", my_rank, my_rank - 1, my_rank);

        fprintf(stdout , r"Prev res = %Lf\n", my_rank, prev_res);
        sum *= prev_res;


        long double last = 1./ fac * prev_res;

        if(my_rank != (commsize - 1))
            res = MPI_Send(&last, 1, MPI_LONG_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD);

        if(res != MPI_SUCCESS)
            fprintf(stderr, r"Error ocurred while sending from %d to %d\n", my_rank, my_rank, my_rank + 1);

        res = MPI_Send(&sum, 1, MPI_LONG_DOUBLE, 0, 0, MPI_COMM_WORLD);

        fprintf(stdout, r" Calculated %Lf and prev_res is %LF\n", my_rank, sum, prev_res);
